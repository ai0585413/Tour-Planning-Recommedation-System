...
def adjust_speed_for_traffic(avg_speed_kmh):
    traffic_factor = random.uniform(0.5, 1.5)  # Random traffic factor between 0.5 and 1.5
    return avg_speed_kmh * traffic_factor


def estimate_fuel_consumption(distance_km, speed_kmh):
    fuel_efficiency = 15  # Assume 15 km per liter as fuel efficiency
...
def calculate_knapsack_score(paths, ratings_and_counts, coordinates_dict, time_in_hours, avg_speed_kmh):
    ...
    for formatted_path, path in paths:
        ...
        max_time_minutes = int(time_in_hours * 60)
        ...
        for i in range(n):
            ...
            traffic_adjusted_speed = adjust_speed_for_traffic(avg_speed_kmh)
            segment_time_minutes = (segment_distance / traffic_adjusted_speed) * 60
            fuel_consumption = estimate_fuel_consumption(segment_distance, traffic_adjusted_speed)
            ...
            # Calculate profit as a combined measure of rating, count, and fuel consumption minus a time penalty
            time_penalty = segment_time_minutes / max_time_minutes  # Normalize time as a penalty
            segment_profit = (avg_rating + avg_count) - time_penalty - fuel_consumption

            segment_profits.append(segment_profit)
            segment_times.append(segment_time_minutes)
            segment_fuel_costs.append(fuel_consumption)
        ...
        total_time_hours = sum(segment_times) / 60

        knapsack_scores.append((formatted_path, optimal_profit, total_time_hours, sum(segment_fuel_costs)))

        # Store only the routes that meet the time constraint
        if total_time_hours <= time_in_hours:
            valid_routes.append((formatted_path, optimal_profit, total_time_hours, sum(segment_fuel_costs)))
...
try:
   ...
   # for node, neighbors in graph.items():
    ...
    knapsack_scores, valid_routes = calculate_knapsack_score(all_paths_titles, ratings_and_counts, coordinates_dict,
    ...
    print("\nHere are your curated route-based recommendations:")
    for formatted_path, score, travel_time, fuel_cost in valid_routes:
        print(
            f"Route: {formatted_path}, Score: {score:.2f}, Travel Time: {travel_time:.2f} hours, Fuel Cost: {fuel_cost:.2f} liters")
