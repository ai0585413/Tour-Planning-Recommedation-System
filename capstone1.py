...
def calculate_mcda_score(paths, ratings_and_counts, coordinates_dict, time_in_hours, avg_speed_kmh):
    scores = []
    valid_routes = []
    ...
    for formatted_path, path in paths:
        ...
        for i in range(len(path) - 1):
            ...
            segment_time = (segment_distance / avg_speed_kmh) * 60

            rating1, count1 = ratings_and_counts.get(place1, (0, 0))
            rating2, count2 = ratings_and_counts.get(place2, (0, 0))

            weighted_rating =  ((rating1 + rating2) / 2)
            weighted_count = ((count1 + count2) / 2)
            avg_score = weighted_rating + weighted_count

            total_distance += segment_distance
            total_time += segment_time
            total_score += avg_score

        if total_time / 60 <= time_in_hours:
            score = total_score - (total_distance / 100) - (total_time / 60)
            scores.append((formatted_path, score, total_time / 60))
            valid_routes.append((formatted_path, score, total_time / 60))

    scores.sort(key=lambda x: x[1], reverse=True)
    valid_routes.sort(key=lambda x: x[1], reverse=True)

    return scores, valid_routes
...
try:
   ...
   # for node, neighbors in graph.items():
    ...
    mcda_scores, valid_routes = calculate_mcda_score(all_paths_titles, ratings_and_counts, coordinates_dict,
